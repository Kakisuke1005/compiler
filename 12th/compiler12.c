...
void statement(void) {
	...
	switch(token.kind){
		case Variable:
			...
			token=nextToken();
			generateCode(LDI,true,token.val);
			generateCode1(ASS);
			token=nextToken();
			if(!checkToken(&token,Semicolon)){
...
void print_statement(void) {
	...
	switch(token.kind){
		case Variable:
			if(containTable(&STable,token.str)){
				generateCode(LD,true,searchTable(&STable,token.str).address);
				generateCode2(LIB,0);
			}
		break;
		default:
...
bool replaceEntry(SymbolTable *st, Entry t) {
	...
	/* lesson11 */
	while(it!=st->tail){
		if(strcmp(*it->token.str,t.token.str/*it->str,t.str*/)==0){
			replaced=true;
			*it=t;
		}
		it++;
